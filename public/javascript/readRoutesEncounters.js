  ...
  * [all encounters from the db,
    ...
    */
    function routeCheckbox(cb_id){
      ...
      tableCellCheckbox.innerHTML = tableCellCheckbox.innerHTML + " <input type='checkbox' id='routeCheckbox" +cb_id+ "' checked=true onclick='routeSelectionForMap("+cb_id+")'>";
    }
 ...
 * @param cb_id
 */
function encounterCheckbox(cb_id) {
  // label the table cell, in which the routeCheckbox will be written, as "tableCellCheckbox"
  let tableCellCheckbox = document.getElementById("confirm"+z);

  // add a routeCheckbox (which calls the function routeSelectionForMap(cb_id) if clicked) to the content of the "tableCellCheckbox"
  tableCellCheckbox.innerHTML = tableCellCheckbox.innerHTML + " <input type='checkbox' id='encounterCheckbox" +cb_id+ "' checked=false onclick='encounterConfirm("+cb_id+")'>";
}
    ...
    class WeatherRequest
    {
      ...
      */
      errorcallback(e) {
        ...
        if (this.status === 404)
        {
          document.getElementById("weather" + (this.id + 1)).innerHTML = "error: no connection to the server";
        ...
        {
          document.getElementById("weather" + (this.id + 1)).innerHTML = "errorcallback: check web-console";
    ...
    class TerrainRequest
    {
      ...
      */
      ...
      errorcallback(e) {
        ...
        if (this.status === 404)
        {
          document.getElementById("country" + (this.id + 1)).innerHTML = "error: no connection to the server";
          document.getElementById("terrain" + (this.id + 1)).innerHTML = "error: no connection to the server";
